...
import dc10.scala.Value.VarA
import dc10.scala.Value.VarC
  ...
  given `3.5.2`: Renderer[Statement, Error, "scala-3.5.2"] =
    new Renderer[Statement, Error, "scala-3.5.2"]:
      ...
      private def renderCaseClassDef[T](cls: Statement.`case class`[T]) =
        renderIndent(cls.tpe.indent) ++ s"case class ${cls.tpe.nme}(${
      ...
      private def renderTraitDef[T](t: Statement.`trait`[T]): String =
        renderIndent(t.tpe.indent) ++ s"""trait ${t.tpe.nme}${t.parent.fold("")(p => s" extends ${renderTypeExpr(p)}")}:
            |${t.body.map(s => render(List(s))).mkString("\n")}""".stripMargin

      private def renderTraitDef[T[_], A](t: Statement.`trait`.`[_]`[T, A]): String =
        renderIndent(t.tpe.indent) ++ s"""trait ${t.tpe.nme}[${renderTypeExpr(t.tparam)}]${t.parent.fold("")(p => s" extends ${renderTypeExpr(p)}")}:
            |${t.body.map(s => render(List(s))).mkString("\n")}""".stripMargin

      private def renderTraitDef[T[_[_]], F[_]](t: Statement.`trait`.`[_[_]]`[T, F]): String =
        renderIndent(t.tpe.indent) ++ s"""trait ${t.tpe.nme}[${renderTypeExpr(t.tparam)}[_]]${t.parent.fold("")(p => s" extends ${renderTypeExpr(p)}")}:
          |${t.body.map(s => render(List(s))).mkString("\n")}""".stripMargin

      private def renderTraitDef[T[_[_], _], F[_], A](t: Statement.`trait`.`[_[_], _]`[T, F, A]): String =
        renderIndent(t.tpe.indent) ++ s"""trait ${t.tpe.nme}[${renderTypeExpr(t.tparamF)}[_], ${renderTypeExpr(t.tparamA)}]${t.parent.fold("")(p => s" extends ${renderTypeExpr(p)}")}:
            |${t.body.map(s => render(List(s))).mkString("\n")}""".stripMargin
        
      private def renderTypeDef[T](t: Statement.`type`[T]): String =
        renderIndent(t.tpe.indent) ++ t.tpe.impl.fold(s"type ${renderTypeExpr(t.tpe)}")(i => s"type ${renderTypeExpr(t.tpe)} = ${renderTypeExpr(i)}")

      private def renderTypeDef[T[_]](t: Statement.`type`.`[_]=>>`[T]): String =
        renderIndent(t.tpe.indent) ++ t.tpe.impl.fold(s"type ${renderTypeExpr(t.tpe)}")(i => s"type ${renderTypeExpr(t.tpe)} = ${renderTypeExpr(i)}")

      private def renderTypeDef[T[_], A](t: Statement.`type`.`[_]`[T, A]): String =
        renderIndent(t.tpe.indent) ++ t.tpe.impl.fold(s"type ${renderTypeExpr(t.tpe)}[${renderTypeExpr(t.tparam)}]")(i => s"type ${renderTypeExpr(t.tpe)}[${renderTypeExpr(t.tparam)}] = ${renderTypeExpr(i)}")
     
      private def renderTypeDef[T[_[_]], F[_]](t: Statement.`type`.`[_[_]]`[T, F]): String =
        renderIndent(t.tpe.indent) ++ t.tpe.impl.fold(s"type ${renderTypeExpr(t.tpe)}[${renderTypeExpr(t.tparam)}[_]]")(i => s"type ${renderTypeExpr(t.tpe)}[${renderTypeExpr(t.tparam)}[_]] = ${renderTypeExpr(i)}")

      private def renderTypeDef[T[_[_], _], F[_], A](t: Statement.`type`.`[_[_], _]`[T, F, A]): String =
        renderIndent(t.tpe.indent) ++ t.tpe.impl.fold(s"type ${renderTypeExpr(t.tpe)}[${renderTypeExpr(t.tparamF)}[_], ${renderTypeExpr(t.tparamA)}]")(i => s"type ${renderTypeExpr(t.tpe)}[${renderTypeExpr(t.tparamF)}[_], ${renderTypeExpr(t.tparamA)}] = ${renderTypeExpr(i)}")
      ...
      def renderValue(value: Value): String =
        value match 
          ...
          case Value.AppPure(i, f, a, t)          => s"${renderValue(f)}(${renderValue(a)})"
      ...
      private def renderValueDef[A](d: Statement.`def`.`0`[A]): String =
        d.value match
          case VarA(indent, nme, tpe) =>
            renderIndent(indent) ++ s"def ${renderValue(d.value)}: ${renderTypeExpr(d.value.tpe)}"
          case VarC(indent, nme, tpe, impl) =>
            renderIndent(indent) ++ s"def ${renderValue(d.value)}: ${renderTypeExpr(d.value.tpe)} = ${renderValue(impl)}"
      ...
      private def renderValueDef[T](d: Statement.field[T]): String =
        d.value match
          case Value.VarA(indent, nme, tpe) => s"${nme}: ${renderTypeExpr(tpe)}"
          case Value.VarC(indent, nme, tpe, impl) => s"${nme}: ${renderTypeExpr(tpe)} = ${renderValue(impl)}"
      ...
      private def renderValueDef[T](d: Statement.`val`[T]): String =
        d.value match
          case Value.VarA(indent, nme, tpe) => renderIndent(indent) ++ s"val ${nme}: ${renderTypeExpr(tpe)}"
          case Value.VarC(indent, nme, tpe, impl) => renderIndent(indent) ++ s"val ${nme}: ${renderTypeExpr(tpe)} = ${renderValue(impl)}"
