...
import dc10.scala.Statement.{ExtensionDef, LibraryDependency, TypeDef, TypeExpr, ValueDef, ValueExpr}
...
object Functions:
  ...
  trait Mixins extends Functions[[A] =>> StateT[ErrorF, (Set[LibraryDependency], List[Statement]), A]]:
 
    extension [A, B] (domain: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[A]])
      @scala.annotation.targetName("fun1T")
      def ==>(
        codomain: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[B]]
      ): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[A => B]] =
        for
          a <- domain
          b <- codomain
          t <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), TypeLevel[A => B]](
    ...
    extension [A, B, C] (domain: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), (TypeExpr[A], TypeExpr[B])])
      @scala.annotation.targetName("fun2T")
      def ==>(
        codomain: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[C]]
      ): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[(A, B) => C]] =
        for
          a <- domain
          b <- codomain
          v <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), TypeLevel[(A, B) => C]](
    ...
    extension [A, B] (fa: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[A]])
      @scala.annotation.targetName("fun1V")
      def ==>(
        f: ValueExpr[A] => StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[B]]
      ): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[A => B]] =
        for
          a <- StateT.liftF[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[A]](fa.runEmptyA)
          b <- f(a)
          t <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), TypeLevel[A => B]](
            Term.TypeLevel.App.Infix(Term.TypeLevel.Lam.Function1Type(), a.value.tpe, b.value.tpe)
          )
          v <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), ValueLevel[A => B]](Term.ValueLevel.Lam.Lam1(a.value, b.value, t))
        yield ValueExpr(v)

    extension [A, B, C] (fa: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), (ValueExpr[A], ValueExpr[B])])
      @scala.annotation.targetName("fun2V")
      def ==>(
        f: (ValueExpr[A], ValueExpr[B]) => StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[C]]
      ): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[(A, B) => C]] =
        for
          a <- StateT.liftF(fa.runEmptyA)
          b <- f(a._1, a._2)
          t <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), TypeLevel[(A, B) => C]](Term.TypeLevel.App.App3(Term.TypeLevel.Lam.Function2Type(), a._1.value.tpe, a._2.value.tpe, b.value.tpe))
          v <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), ValueLevel[(A, B) => C]](Term.ValueLevel.Lam.Lam2(a._1.value, a._2.value, b.value, t))
        yield ValueExpr(v)

    def EXT[G[_], B](
      func: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), G[B]]
    )(using sp: SourcePos): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), G[B]] =
      for
    ...
    @scala.annotation.targetName("forOption")
    def FOR[A](f: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[A]])(using sp: SourcePos): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[Option[A]]] =
      for
        ((d, l), a) <- StateT.liftF(f.runEmpty)
        t <- OPTION(StateT.pure(TypeExpr(a.value.tpe)))
        v <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), ValueLevel[Option[A]]](Term.ValueLevel.Blc.ForComp(l, a.value, t.tpe))
      yield ValueExpr(v)

    extension [G[_], A] (nme: String)
      def <--(ff: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[G[A]]]): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), ValueExpr[A]] =
        for
          g <- ff
          t <- StateT.liftF[ErrorF, (Set[LibraryDependency], List[Statement]), TypeLevel[A]](g.value.tpe match
            ...
            case dc10.scala.Symbol.Term.TypeLevel.Var.__() => Left(List(Error("Not of kind type -> type")))
            ...
            case dc10.scala.Symbol.Term.TypeLevel.Var.UserDefinedType(nme, tparams, impl) => Left(List(Error("Not of kind type -> type")))
          )
          i <- StateT.liftF[ErrorF, (Set[LibraryDependency], List[Statement]), ValueLevel[A]](g.value.findImpl.fold(Left(List(Error(""))))(i => i match
            ...
            case dc10.scala.Symbol.Term.ValueLevel.Var.UserDefinedValue(nme, tpe, tparams, impl) => Left(List(Error("Not of kind type -> type")))
          )
          )
          v <- StateT.liftF[ErrorF, (Set[LibraryDependency], List[Statement]), Term.ValueLevel.Var.UserDefinedValue[A]](
                Right(Term.ValueLevel.Var.UserDefinedValue(nme, t, Nil, Some(i))))
    ...
    def MATCHTYPES[T[_], A, B](
      nme: String,
      arg: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[A]],
      cases: TypeExpr[A] => StateT[ErrorF, (Set[LibraryDependency], List[Statement]), B]
    )(using sp: SourcePos): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[T[A]]] =
      for
        ...
        f <- StateT.pure[ErrorF, (Set[LibraryDependency], List[Statement]), Term.TypeLevel.Var.UserDefinedType[T[A]]](Term.TypeLevel.Var.UserDefinedType(nme, Nil, None))
        t <- StateT.liftF(a.tpe match
          case u@Term.TypeLevel.Var.UserDefinedType(_, _, _) => Right(Term.TypeLevel.App.App1(f, a.tpe))
    ...
    def CASE[A, B](f: StateT[ErrorF, (Set[LibraryDependency], List[Statement]), TypeExpr[A => B]])(using sp: SourcePos): StateT[ErrorF, (Set[LibraryDependency], List[Statement]), Unit] =
      for
        a <- f
