...
object Concurrent:
  ...
  extension [G[_]] (s: `Type.Expr: (*→*)→*→* *→*`[SignallingRef, G])
    // def of[G[_], A](initial: `Value.Expr: *`[A])(using C: Concurrent[G]): G[SignallingRef[G, A]] =
    def of[A](initial: `Value.Expr: *`[A]): `Value.AppDot.1: *→* ((*→*)→*→* *→* *)`[G, SignallingRef, G, A, A] =
      s.dot(
        `Value.Def.1: *→*→* * (*→* ((*→*)→*→* *→* *))`(
          0,
          `DefSym.1`("of", `Value.Val: *`(0, ValSym("initial"), initial.tpe, None)),
