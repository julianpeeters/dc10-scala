  ...
  trait Mixins extends Objects[[A] =>> StateT[ErrorF, List[Statement], A]]:
    ...
    def OBJECT[T, A](name: String, contents: StateT[ErrorF, List[Statement], A]): StateT[ErrorF, List[Statement], List[Statement]] =
      for
        ...
        o <- StateT.pure[ErrorF, List[Statement], Symbol.Object[T]](Symbol.Object(None, name, None, c.map(s => s.addIndent)))
        d <- StateT.pure[ErrorF, List[Statement], ObjectDef](ObjectDef(o, 0))
        _ <- StateT.modifyF[ErrorF, List[Statement]](ctx => ctx.ext(d))
      yield c

    def OBJECT[T, A](name: String, parent: TypeExpr[T], contents: StateT[ErrorF, List[Statement], A]): StateT[ErrorF, List[Statement], List[Statement]] =
      for
        c <- StateT.liftF[ErrorF, List[Statement], List[Statement]](contents.runEmptyS)
        o <- StateT.pure[ErrorF, List[Statement], Symbol.Object[T]](Symbol.Object(None, name, Some(parent.tpe), c.map(s => s.addIndent)))
        d <- StateT.pure[ErrorF, List[Statement], ObjectDef](ObjectDef(o, 0))
        _ <- StateT.modifyF[ErrorF, List[Statement]](ctx => ctx.ext(d))
      yield c
