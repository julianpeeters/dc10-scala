...
import dc10.scala.ctx.{CompilerError, ErrorF, IdentifierSymbolExpected, ext}
  ...
  trait Mixins extends TemplateTypes[
      ...
      for
        ...
        v <- StateT.liftF[ErrorF, List[Statement], ValueExpr[A => T]](
          a.value.tail.value match
            case Term.ValueLevel.App1(_, _, _)          => Left(scala.List(IdentifierSymbolExpected(a.value.tail.value)))
            case Term.ValueLevel.AppCtor1(_, _, _)      => Left(scala.List(IdentifierSymbolExpected(a.value.tail.value)))
            case Term.ValueLevel.AppVargs(_, _, vargs*) => Left(scala.List(IdentifierSymbolExpected(a.value.tail.value)))
            case Term.ValueLevel.Lam1(_, _, _)          => Left(scala.List(IdentifierSymbolExpected(a.value.tail.value)))
            ...
            case Term.ValueLevel.Var.UserDefinedValue(qnt, nme, tpe, impl) => Right[List[CompilerError], Statement.ValueExpr[A => T]](ValueExpr[A => T](
    ...
    def FIELD[T](
      ...
      for
        ...
        d <- StateT.pure[ErrorF, List[Statement.ValueDef], ValueDef](ValueDef.Val(0, v))
