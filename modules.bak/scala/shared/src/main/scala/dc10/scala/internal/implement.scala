...
object implement:
  ...
  extension [T] (t: `Type: x`[T])
    ...
    def assign[F[_]](rhs: `Type: x→x`[F]): Either[List[Error], `Type.Var: x→x`[F]] =
      t match
        case `Type.App[_]`(in, tfun, aarg) => Left(List(Error(s"Type is not assignable ${t}")))
        case `Type.App[_[_]]`(in, tfun, farg) => Left(List(Error(s"Type is not assignable ${t}")))
        case `Type.App[_, _]`(in, tfun, aarg, barg) => Left(List(Error(s"Type is not assignable ${t}")))
        ...
        case `Type.AppInfix[_, _, _]`(in, tfun, aarg, barg, carg) => Left(List(Error(s"Type is not assignable ${t}")))
        case `Type.AppInfix[_, _, _, _]`(in, tfun, aarg, barg, carg, darg) => Left(List(Error(s"Type is not assignable ${t}")))
        case `Type.Var: x`(in, nme, impl) => impl.fold(Right(`Type.Var: x→x`(in, nme, Some(rhs), () => scala.Nil)))(_ => Left(List(Error(s"Type is already assigned ${t}"))))
        case `Type.Bot: x`(in) => Left(List(Error(s"Type is not assignable ${t}")))
  ...
  extension [T] (value: `Value: x`[T])
    def findImpl: Option[`Value: x`[T]] =
      value match
        ...
        case `Value.App.Vargs: x`(in, fun, tpe, vargsx) => Some(value)
        ...
        case `Value.AppForComp: x→x x`(in, l, r, t) => Some(value)
        case `Value.App.Match`(in, v, t, l) => Some(value)
        case `Value.App.0: x→x`(in, fun, targ, tpe) => Some(value)
        case `Value.App.0: (x→x)→x→x`(in, fun, targf, targa, tpe) => Some(value)
        case `Value.App.0: (x→x)→x→x`(in, fun, targf, targa, targb, tpe) => Some(value)
        case `Value.Lam.1: x→x→x x x`(in, a, b, t) => Some(value)
        case `Value.Lam.2: x→x→x→x x * x`(in, a1, a2, b, t) => Some(value)
        case `Value.Lam.3: x→x→x→x→x x * x x`(in, a1, a2, a3, b, t) => Some(value)
